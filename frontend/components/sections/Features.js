...
const CardItem = ({ image, title, desc }) => {
  ...
  return (
    <>
      <div
        className="flex flex-col items-start bg-white bg-opacity-50 justify-between h-60
         rounded-3xl mx-8 md:mx-1 p-8 md:p-4 sm:text-xs sm:items-start sm:justify-start"
        onMouseEnter={handleHover}
        onMouseLeave={handleHoverEnd}
      >
        <motion.div animate={controls} className="sm:w-8 sm:h-8">
          <Image src={image} height={76} width={73} alt="wallet" />
        </motion.div>

        <p className="text-lg font-extrabold text-primary sm:mt-4">{title}</p>
        <p className="text-sm  text-dark font-thin sm:text-xs sm:mt-2">
          {desc}
        </p>
      </div>
    </>
...
const Features = () => {
  return (
    <div>
      <div
        id="features"
        className="w-full flex flex-col items-center justify-center mt-24 relative sm:mt-10 "
      >
      ...
      <div
        className="grid grid-cols-4 gap-y-8 md:grid-cols-2 md:gap-x-0 lg:grid-cols-2 lg:gap-8 md:gap-y-16 sm:sm-features
          rounded-3xl
        sm:bg-lightGreen sm:bg-opacity-20
       "
      >
